...
export function registerRoutes(app) {
    ...
    app.post("/api/decks/save", requireAuth, async (req, res) => {
        try {
            ...
            const total = cards.reduce((acc, c) => acc + Number(c.count ?? 0), 0);
            if (total !== 9)
                return res.status(400).json({ error: "Deck must be exactly 9 cards" });
