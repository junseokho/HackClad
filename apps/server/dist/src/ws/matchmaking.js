...
async function createPvpRoom(roomId, selected) {
    ...
    for (let idx = 0; idx < selected.length; idx++) {
        ...
        const characterId = (user?.selectedCharacterId ?? (await prisma.user.findUnique({ where: { id: c.user.id } }))?.selectedCharacterId);
        ...
        if (!deck) {
            ...
            if (enhancedPick)
                picked.push({ cardId: enhancedPick.cardId, count: 1 });
...
export function joinMatchmaking(client, mode, size) {
...
export function leaveMatchmaking(client) {
    if (!client.matchKey)
        return;
    ...
    broadcast(q, { type: "match:status", status: "searching", current: q.length, needed: needed, mode: key });
