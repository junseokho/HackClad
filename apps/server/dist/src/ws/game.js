...
const CARD_DB = SAMPLE_PLAYER_CARDS.reduce((acc, c) => {
    acc[c.code] = {
        code: c.code,
        name: c.name,
        mpCost: c.mpCost,
...
const LUNA_SUPPORT_DIVINATION_CARDS = new Set([
...
(async () => {
    try {
        ...
        for (const c of cards) {
            ...
            const range = Array.isArray(effect.range) ? effect.range.map((r) => ({ x: r.x ?? 0, y: r.y ?? 0 })) : undefined;
            const entry = {
                code: c.code,
                name: c.name,
...
const BASIC_DMG_LIMITS = {
    CH_ROSETTE_DELTA: 1,
    CH_FLARE_DELTA: 2,
    CH_LUNA_DELTA: 2,
    CH_MIA_DELTA: 1
};
const BASIC_MOVE_LIMITS = {
    CH_FLARE_DELTA: 1,
    CH_LUNA_DELTA: 1,
    CH_MIA_DELTA: 3
};
const BASIC_MP_LIMITS = {
    CH_MIA_DELTA: 1
};
function basicLimitsFor(p) {
...
function recordDamage(attacker, amount) {
...
const REACTION_CARD_OVERRIDES = new Set([
...
function isReactionCardDef(def, code) {
    const cardType = (def.cardType ?? "").toLowerCase();
    if (cardType === "reaction")
        return true;
    if (typeof def.guard === "number")
        return true;
...
function shardKey(pos) {
    return `${pos.x},${pos.y}`;
}
function grantShards(p, amount) {
    if (amount <= 0)
        return;
...
function collectShards(state, p, pos) {
    if (!pos)
        return;
...
function applyDamageToPlayer(state, p, damage) {
...
function bossPosition(state) {
    return state.boss?.position ?? { x: 0, y: 0 };
}
function ensureFacing(f) {
    if (f === "N" || f === "E" || f === "S" || f === "W")
        return f;
    return "N";
}
function isBossInRange(state, attacker, range) {
...
function hasMaelstromFormation(p) {
    return Array.isArray(p.discard) && p.discard.includes("HacKClaD_Flare_Delta_Cards_MaelstromFormation");
}
function maybeTriggerMaelstrom(state, p, cpSpent) {
    if (cpSpent > 0)
        return;
    if (!hasMaelstromFormation(p))
        return;
    p.cp = (p.cp ?? 0) + 1;
}
function isFacingBossFront(state, attacker) {
    if (!attacker?.position)
        return false;
    const bossPos = bossPosition(state);
...
function isFacingBossBack(state, attacker) {
    if (!attacker?.position)
        return false;
    const bossPos = bossPosition(state);
    const bossFacing = ensureFacing(state.boss?.facing ?? "N");
...
function bossTopIsVoltageOneOrEmpty(state) {
    const top = state.boss?.deck?.[0];
    if (!top)
        return true;
    return bossTopVoltage(state) === 1;
}
function performDivination(state, p) {
    const top = state.boss?.deck?.[0];
    if (!top) {
        p.mp = (p.mp ?? 0) + 1;
        return;
...
function repelBoss(state, from) {
    ...
    const occupiedByPlayer = state.players.some((pl) => pl.position && pl.position.x === next.x && pl.position.y === next.y);
    if (!occupiedByPlayer) {
...
function performBossAttack(state, p, range, atk, extra) {
    const targets = range
...
function computeReactionOrder(state) {
    return state.players
        .slice()
        .sort((a, b) => (a.chosenSlot ?? 10) - (b.chosenSlot ?? 10) || a.userId.localeCompare(b.userId))
        .map((p) => p.userId);
}
function computeAttackTargets(pos, facing, offsets) {
    return offsets.map((o) => {
        const r = rotateOffset(o, facing);
        return wrapPosition({ x: pos.x + r.x, y: pos.y + r.y });
    });
}
function eligiblePlayersForAttack(state, pending) {
    const targets = computeAttackTargets(pending.pos, pending.facing, pending.offsets);
    return state.players
        .filter((pl) => pl.position && targets.some((t) => t.x === pl.position.x && t.y === pl.position.y))
        .map((pl) => pl.userId);
}
function legionTargetsFromPending(state, pending) {
    const adjOffsets = [
        { x: 0, y: 1 },
        { x: 0, y: -1 },
        { x: 1, y: 0 },
    ...
    for (const head of pending.attackers) {
        for (const off of adjOffsets) {
            ...
            const hit = state.players.find((pl) => pl.position && pl.position.x === t.x && pl.position.y === t.y);
...
function eligiblePlayersForLegion(state, pending) {
    return legionTargetsFromPending(state, pending).map((t) => t.userId);
}
function applyLegionPendingDamage(state) {
    const pending = state.legionPending;
    if (!pending)
        return;
...
function requestChoice(state, roomClients, targetUserId, type, data, prompt, defaultValue, opts) {
    clearChoice(state);
    const choiceId = `${type}-${Date.now()}-${Math.random()}`;
    const target = roomClients.find((c) => c.user.id === targetUserId);
...
function applyChoice(state, roomClients, choiceId, value, client) {
    const pending = state.pendingChoice;
    if (!pending || pending.id !== choiceId)
        return;
    clearChoice(state);
    ...
    if (pending.type === "nightParade") {
        if (value === true) {
            ...
            if (top) {
                ...
                if (topType !== "reaction") {
                    ...
                    if (idxDis >= 0)
                        p.discard.splice(idxDis, 1);
                    p.hand.push(top);
                    const fakeClient = client ?? roomClients.find((c) => c.user.id === p.userId);
                    if (fakeClient) {
        ...
        if (value === true && Array.isArray(pending.data?.targets)) {
            const target = pending.data.targets.find((t) => !isOccupied(state, p, t));
            if (target) {
        ...
        if (value === true && (p.mp ?? 0) >= 1) {
            if (flipTrapFaceUp(p)) {
                ...
                const target = client ?? roomClients.find((c) => c.user.id === p.userId);
                if (target) {
        ...
        const discardOptions = Array.isArray(pending.data?.discardOptions) ? pending.data.discardOptions : [];
        const enhancedOptions = Array.isArray(pending.data?.enhancedOptions) ? pending.data.enhancedOptions : [];
        const discardPick = value?.discard;
        const enhancedPick = value?.enhanced;
        if (!discardOptions.includes(discardPick) || !enhancedOptions.includes(enhancedPick))
            return;
        ...
        if (Array.isArray(p.enhancedDeck)) {
            p.enhancedDeck = p.enhancedDeck.filter((c) => c !== enhancedPick);
        }
...
function wrapCoord(v, size = 5) {
    const half = Math.floor(size / 2);
    return ((((v + half) % size) + size) % size) - half;
}
function wrapPosition(pos, size = 5) {
    return { x: wrapCoord(pos.x, size), y: wrapCoord(pos.y, size) };
}
function manhattanWrapped(a, b, size = 5) {
...
function attemptDirectionalMove(state, p, dir, opts) {
...
function movePlayerTo(state, p, target) {
    if (!p.position)
        return false;
    if (p.cannotMoveThisTurn)
        return false;
...
function currentVoltage(state) {
    return state.boss?.voltage ?? state.boss?.voltageTier ?? state.voltage ?? 1;
}
function isOccupied(state, me, pos) {
    if (state.boss?.position && state.boss.position.x === pos.x && state.boss.position.y === pos.y)
        return true;
    if (Array.isArray(state.legions)) {
        const hasLegion = state.legions.some((l) => l.position.x === pos.x && l.position.y === pos.y);
        if (hasLegion)
            return true;
    }
    return state.players.some((pl) => pl !== me && pl.position && pl.position.x === pos.x && pl.position.y === pos.y);
}
function placeAtEntry(state, p) {
    const ENTRY_POINTS = [
        { x: -2, y: -1 }, // Entry A
        { x: 2, y: 1 }, // Entry B
        { x: -1, y: 2 }, // Entry C
    ...
    for (const ep of ENTRY_POINTS) {
        const occupied = state.players.some((pl) => pl !== p && pl.position && pl.position.x === ep.x && pl.position.y === ep.y);
        if (!occupied) {
            p.position = { ...ep };
            p.facing = "N";
            collectShards(state, p, p.position);
...
function availableEntries(state, p) {
    const ENTRY_POINTS = [
        { x: -2, y: -1 },
        { x: 2, y: 1 },
        { x: -1, y: 2 },
...
function setLegion(state, pos, type, facing) {
    state.legions = state.legions ?? [];
    const wrapped = wrapPosition(pos);
    state.legions = state.legions.filter((l) => !(l.position.x === wrapped.x && l.position.y === wrapped.y));
    state.legions.push({ type, position: wrapped, facing, hp: currentVoltage(state) });
}
function rewardForLegionDefeat(state, legion, attacker) {
...
function damageLegionAt(state, pos, damage, attacker) {
    if (!Array.isArray(state.legions))
        return;
    const idx = state.legions.findIndex((l) => l.position.x === pos.x && l.position.y === pos.y);
    if (idx < 0)
        return;
...
function getPlayer(state, userId) {
    return state.players.find((x) => x.userId === userId);
}
function getActionState(roomId) {
    const state = rooms.get(roomId);
    if (!state || state.phase !== "action" || state.finished)
        return null;
    return state;
}
function getActionContext(roomId, userId, opts) {
    ...
    if (!isMyTurn && !(allowReaction && isReaction))
        return null;
    return { state, player, isReaction, isMyTurn };
}
export function handlePvpEnter(client, roomId, pos, roomClients) {
    const state = getActionState(roomId);
    if (!state)
        return;
...
function placeIfNeeded(state, userId) {
    ...
    const occupied = new Set(state.players
        ...
        .map((pl) => `${pl.position.x},${pl.position.y}`));
    const slot = spawnCandidates.find((c) => !occupied.has(`${c.x},${c.y}`)) ?? spawnCandidates[0];
    p.position = slot;
    p.facing = "N";
    collectShards(state, p, p.position);
}
function reformIfNeeded(state, p) {
    if (p.deck.length > 0)
        return true;
    if (p.discard.length === 0)
        return false;
    if (p.reformUsedThisRound)
        return false;
    const enhancedPool = Array.isArray(p.enhancedDeck) ? p.enhancedDeck : [];
    if (enhancedPool.length > 0 && p.discard.length > 0) {
        const pending = state.pendingChoice;
        if (pending?.type === "reformSwap" && pending.targetUserId === p.userId)
            return false;
        ...
        requestChoice(state, roomClients, p.userId, "reformSwap", { discardOptions, enhancedOptions }, "덱 재구성: 버린 카드 1장을 선택해 제거하고, 확장 덱 카드 1장을 추가하세요.", null, { options: { type: "reformSwap", discardOptions, enhancedOptions }, timeoutMs: 0 });
        return false;
    }
    p.deck.push(...p.discard);
    p.deck = shuffle(p.deck);
...
function drawCard(state, p) {
    if (p.deck.length === 0)
        reformIfNeeded(state, p);
    if (p.deck.length === 0)
        return false;
    p.hand.push(p.deck.shift());
    return true;
}
function drawTo3(state, p) {
...
function canPayForCard(p, d) {
    return (p.mp ?? 0) >= d.mpCost;
}
function applyCardEconomy(p, d) {
    p.mp -= d.mpCost;
    p.mp += d.gainMp;
    p.cp += d.gainCp;
}
function discardFromHand(p, cardCode) {
...
function allSlotsChosen(state) {
    return state.players.every((p) => typeof p.chosenSlot === "number");
}
function allEnhancedPicked(state) {
    return state.players.every((p) => p.needsEnhancedPick !== true);
}
function turnFacing(current, dir) {
    if (dir === "left") {
        if (current === "N")
            return "W";
        if (current === "W")
...
function refillBossDeckIfNeeded(state) {
    if (state.boss.deck.length > 0)
        return;
...
function executeBossActions(state, card, startIdx, pos, facing, damage) {
    for (let idx = startIdx; idx < card.actions.length; idx++) {
        ...
        if (action.type === "move") {
            ...
            const hitPlayers = state.players.filter((pl) => pl.position && pl.position.x === pos.x && pl.position.y === pos.y);
            ...
            if (kind) {
                for (const off of action.offsets) {
                    ...
                    const hitPlayers = state.players.filter((pl) => pl.position && pl.position.x === summonPos.x && pl.position.y === summonPos.y);
                    if (hitPlayers.length > 0) {
                        for (const pl of hitPlayers) {
                            applyDamageToPlayer(state, pl, damage);
                        }
            ...
            const inRange = state.players.filter((pl) => pl.position && targets.some((t) => t.x === pl.position.x && t.y === pl.position.y));
            if (inRange.length === 0) {
                continue;
            }
...
function resolveBossCard(state, step) {
    ...
    const facing = state.boss.facing ?? "N";
...
function performLegionAttack(state) {
    const voltage = currentVoltage(state);
    if (voltage <= 0)
        return false;
    const heads = (state.legions ?? []).filter((l) => l.type === "head");
    const adjOffsets = [
        { x: 0, y: 1 },
        { x: 0, y: -1 },
        { x: 1, y: 0 },
...
function resolveLegionAttack(state) {
    return performLegionAttack(state);
}
function resumeBossAfterReaction(state) {
    const pending = state.bossPending;
    if (!pending)
        return;
...
function refreshReactionEligibility(state) {
    ...
    if (ctxType === "bossAttack") {
        ...
        const order = computeReactionOrder(state);
        const eligible = eligiblePlayersForAttack(state, pending).filter((id) => order.includes(id));
        const rangeChanged = eligible.length !== reaction.eligible.length || eligible.some((id) => !reaction.eligible.includes(id));
        if (rangeChanged) {
            reaction.passed = [];
        }
        ...
        if (eligible.length === 0) {
            ...
            for (const id of targets) {
                ...
                if (target)
                    applyDamageToPlayer(state, target, pending.damage);
            }
...
function markDoneAndAdvance(state) {
...
function endRoundOrScore(state) {
    if (state.round >= 9) {
        state.phase = "scoring";
        state.finished = true;
        return;
...
function computeFinalScores(state) {
    return state.players
        ...
        const cardVp = allCodes.reduce((acc, code) => {
...
// handlers
// --------------------
export function handleGameSubscribe(client, roomId, _roomClients) {
    const state = rooms.get(roomId);
    if (!state) {
...
export function handleGameChoice(client, roomId, choiceId, value, roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    const pending = state.pendingChoice;
    if (!pending || pending.id !== choiceId)
        return;
    if (pending.targetUserId !== client.user.id)
        return;
    applyChoice(state, roomClients, choiceId, value, client);
}
export function handlePvpReady(client, roomId, _roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    const p = getPlayer(state, client.user.id);
...
export function handlePvpChooseSlot(client, roomId, slot, turnCardCode, spawn, returnCardCode, moveTarget, _roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    if (state.phase !== "draft" || state.finished)
        return;
    if (!allEnhancedPicked(state))
        return;
    if (slot < 1 || slot > 4)
        return;
    const taken = state.players.some((p) => p.chosenSlot === slot);
    if (taken)
        return;
    const p = getPlayer(state, client.user.id);
    if (!p)
    ...
    const unchosen = state.players.filter((pl) => typeof pl.chosenSlot !== "number");
    const nextStandby = Math.min(...unchosen.map((pl) => pl.standbySlot ?? 99));
    if ((p.standbySlot ?? 99) !== nextStandby)
        return;
    // Cannot pick the mirrored forbidden slot (turnSlot != 5 - standbySlot)
    const forbiddenTurnSlot = 5 - (p.standbySlot ?? 0);
    if (slot === forbiddenTurnSlot)
        return;
    ...
    if (!p.position) {
        ...
        const valid = spawn &&
            ...
            !state.players.some((pl) => pl.position && pl.position.x === spawn.x && pl.position.y === spawn.y);
        if (!valid)
            return;
        p.position = spawn;
        p.facing = "N";
    ...
    if (slot === 1) {
        ...
        if (p.hand.length > 0) {
            const idx = returnCardCode && p.hand.includes(returnCardCode) ? p.hand.indexOf(returnCardCode) : p.hand.length - 1;
...
export function handlePvpPickEnhanced(client, roomId, cardCode, _roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    if (state.finished)
    ...
    if (!p)
        return;
    if (p.needsEnhancedPick !== true)
        return;
    if (!Array.isArray(p.enhancedOptions) || !p.enhancedOptions.includes(cardCode))
        return;
    ...
    p.enhancedOptions = [];
    if (Array.isArray(p.enhancedDeck)) {
        p.enhancedDeck = p.enhancedDeck.filter((c) => c !== cardCode);
    }
    progress(state);
    broadcastState(state);
}
export function handlePvpPlayCard(client, roomId, cardCode, dir, _roomClients) {
    ...
    if (code === "HacKClaD_Mia_Delta_Cards_Heelstomp" && p.miaHeelstompExtra === undefined) {
        const hasKunai = p.hand.some((c) => c === "HacKClaD_Mia_Delta_Cards_Kunai" || c === "HacKClaD_Mia_Delta_Cards_Kunai2");
        if (hasKunai) {
            requestChoice(state, _roomClients, client.user.id, "miaHeelstompExtra", { cardCode, dir }, "Heelstomp: 쿠나이를 버리고 공격력 +1 및 넉백을 얻을까요?", false);
            return;
        }
    }
    ...
    if (isAttackType && baseRange.length > 0 && baseAtk > 0) {
        ...
        if (code === "HacKClaD_Luna_Delta_Cards_Condemn" ||
        ...
        if (code === "HacKClaD_Mia_Delta_Cards_Heelstomp") {
            ...
            if (useBonus) {
                const kunaiIdx = p.hand.findIndex((c) => c === "HacKClaD_Mia_Delta_Cards_Kunai" || c === "HacKClaD_Mia_Delta_Cards_Kunai2");
                if (kunaiIdx >= 0) {
        ...
        if (canResolveBossOnly) {
            for (let i = 0; i < multistrike; i++) {
                performBossAttack(state, p, baseRange, atk, shouldRepel && p.position
        ...
        if (code === "HacKClaD_Mia_Delta_Cards_WeaponForaging") {
            ...
            const recovered = p.discard.filter((c) => kunaiCodes.includes(c));
            if (recovered.length > 0) {
                p.discard = p.discard.filter((c) => !kunaiCodes.includes(c));
                p.hand.push(...recovered);
            }
        }
        if (code === "HacKClaD_Rosette_Delta_Cards_Impale" && (p.cp ?? 0) >= 1 && p.position) {
            requestChoice(state, _roomClients, client.user.id, "impale", {}, "Impale: spend 1 CP to turn the Clad to face you?", false);
        }
        if (code === "HacKClaD_Luna_Delta_Cards_Thunderstep" && destroyedLegions.length > 0) {
            const uniqueTargets = destroyedLegions.filter((pos, idx, arr) => arr.findIndex((t) => t.x === pos.x && t.y === pos.y) === idx);
            const movableTargets = uniqueTargets.filter((pos) => !isOccupied(state, p, pos));
            if (movableTargets.length > 0) {
                requestChoice(state, _roomClients, client.user.id, "lunaThunderstepMove", { targets: movableTargets }, "Thunderstep: move to the defeated Legion's space?", false);
            }
        }
        ...
        if (code === "HacKClaD_Luna_Delta_Cards_Invocation") {
            const supportIdx = p.discard.findIndex((c) => {
                const def = cardDef(c);
        ...
        if (code === "HacKClaD_Luna_Delta_Cards_AuxillaryMana") {
            if ((p.mp ?? 0) >= 1) {
                requestChoice(state, _roomClients, client.user.id, "lunaAuxDivination", {}, "Auxillary Mana: spend 1 MP to perform Divination?", false);
            }
        }
        if (code === "HacKClaD_Luna_Delta_Cards_SoaringHeights") {
            ...
            if (targets.length > 0) {
                requestChoice(state, _roomClients, client.user.id, "lunaSoaringHeightsMove", { targets }, "Soaring Heights: move to an adjacent space?", false);
            }
        }
        ...
        if (code === "HacKClaD_Mia_Delta_Cards_AuxillaryMana") {
            if ((p.mp ?? 0) >= 1) {
                if (ensureMiaTraps(p).some((t) => !t)) {
                    requestChoice(state, _roomClients, client.user.id, "miaAuxTrapFlip", {}, "Auxillary Mana: MP 1을 지불해 코니베어 트랩을 앞면으로 뒤집을까요?", false);
                }
                else {
                    sendError(client.ws, "No face-down Conibear Trap to flip.");
                }
        ...
        if (code === "HacKClaD_Rosette_Delta_Cards_HundredDemons") {
            requestChoice(state, _roomClients, client.user.id, "nightParade", {}, "Discard the top card; you may play it. Gain an extra Crack use this turn. Proceed?", false);
            broadcastState(state);
...
export function handlePvpMiaTrapAttack(client, roomId, target, _roomClients) {
    ...
    if (target?.kind === "boss") {
        ...
        const legion = (state.legions ?? []).some((l) => l.position.x === wrapped.x && l.position.y === wrapped.y);
        if (!legion) {
...
// --------------------
export function handlePvpBasicAction(client, roomId, action, dir, discardCard, _roomClients) {
    const ctx = getActionContext(roomId, client.user.id);
    if (!ctx)
        return;
    const { state, player: p } = ctx;
...
export function handlePvpCpAction(client, roomId, actionId, target, dir, _roomClients) {
    const ctx = getActionContext(roomId, client.user.id);
    if (!ctx)
        return;
    const { state, player: p, isReaction } = ctx;
...
export function handlePvpReact(client, roomId, kind, payload, _roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    ...
    if (kind === "pass") {
        ...
        const next = state.reaction.eligible.find((id) => !state.reaction.passed.includes(id));
        if (next) {
    ...
    if (kind === "playCard") {
        ...
        if ((d.range?.length ?? 0) > 0 && reactionAtk > 0) {
            performBossAttack(state, p, d.range ?? [], reactionAtk, p.position ? { repelFrom: p.position, forceBossHit } : { forceBossHit });
        }
        if (UNYIELDING_CARDS.has(code)) {
            activateUnyielding(p);
        }
...
export function handlePvpCrack(client, roomId, dir, steps, moveTarget, roomClients, opts) {
...
export function handlePvpEndTurn(client, roomId, _roomClients) {
...
export function handleAdvancePhaseDebug(roomId, _roomClients) {
    const state = rooms.get(roomId);
    if (!state)
        return;
    if (state.finished)
