import { cardByCode } from "./state.js";
...
export function ensurePhaseProgress(s) {
    if (s.finished)
        return;
    if (!allReady(s))
        return;
...
export function setReady(s, userId) {
...
export function playCard(s, userId, cardCode) {
    if (s.finished)
        return;
    if (s.phase !== "action")
        return;
...
export function endTurn(s, userId) {
    if (s.finished)
        return;
    if (s.phase !== "action")
        return;
...
export function advancePhaseDebug(s) {
    if (s.finished)
        return;
    if (s.phase === "forecast")
        s.phase = "draw";
...
export function toClientState(s) {
